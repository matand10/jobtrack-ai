...
export function StatCard({ icon: Icon, label, sub, tileClassName, value }: StatCardProps) {
  return (
    <Card className="flex flex-col gap-3 p-4" aria-label={`${label}: ${value}${sub ? ` ${sub}` : ''}`}>
      <div className="flex items-center justify-between gap-3">
        <p className="text-[12.5px] font-medium tracking-[0.02em] text-app-text-3 uppercase">{label}</p>
      ...
      <div className="flex items-baseline gap-2">
        ...
        {sub ? <p className="text-xs font-medium text-app-ai-600">{sub}</p> : null}
      </div>
    </Card>
  )
}
