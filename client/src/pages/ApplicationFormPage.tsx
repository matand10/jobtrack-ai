...
import { useApplication, useCreateApplication, useUpdateApplication } from '../hooks/application.hooks'
...
function CreateApplicationPage({ user }: { user: { name: string; email: string } }) {
  ...
  return (
    <AppShell title="Add Application" user={user}>
      <Breadcrumbs items={[{ label: 'Applications', href: '/applications' }, { label: 'New application' }]} />
      <div className="mb-6">
        <h1 className="text-[28px] font-bold tracking-[-0.02em] text-app-text">Add a new application</h1>
        <p className="mt-1 text-[13px] text-app-text-3">Track a new role you're applying to or just exploring.</p>
      </div>
...
function EditApplicationPage({ id, user }: { id: string; user: { name: string; email: string } }) {
  const navigate = useNavigate()
  const [serverError, setServerError] = useState<string | null>(null)
  ...
  return (
    <AppShell title="Edit application" user={user}>
      <Breadcrumbs items={[{ label: 'Applications', href: '/applications' }, { label: 'Edit application' }]} />
      <div className="mb-6">
        <h1 className="text-[28px] font-bold tracking-[-0.02em] text-app-text">Edit application</h1>
        <p className="mt-1 text-[13px] text-app-text-3">Update role details, status, and job description.</p>
      </div>
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_320px]">
        <ApplicationForm
          mode="edit"
...
export function EditApplicationPage() {
  ...
  return <EditApplicationPage id={id} user={user} />
}
